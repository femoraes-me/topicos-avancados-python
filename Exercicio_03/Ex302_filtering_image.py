...
def saltAndPepperNoise(image, density):
    ...
    noised [(( np. random . rand (h, w)) < density )] = [-1]
...
def gaussianNoise(image, mu, sigma):
    ...
    # TODO: Your code here
    noise = to_uint8(np.random.normal(mu, sigma, (h, w)))    
    noised = cv2.add(noised, noise)

    # stats.normaltest(noised)
...
def uniformNoise(image, low, high):
    ...
    # TODO: Your code here
    noise = to_uint8(np.random.uniform(low, high, (h, w)))    
    noised = cv2.add(noised, noise)
...
def saltAndPepperFilter(image, n=3):
    ...
    # TODO: Your code here
    # cv2.fastNlMeansDenoisingMulti(filtered, 2 ,n, None, w , h - 1, 35)
    kernel = cv2.getStructuringElement(cv2.MORPH_OPEN, (n, n))
    filtered = cv2.morphologyEx(filtered, cv2.MORPH_OPEN, kernel, iterations = 1)
    # kernel = cv2.getStructuringElement(cv2.MORPH_CLOSE, (n, n))
...
def uniformFilter(image, n=5):
    ...
    # TODO: Your code here
    kernel = cv2.getStructuringElement(cv2.MORPH_OPEN, (n, n))
    filtered = cv2.morphologyEx(filtered, cv2.MORPH_HITMISS, kernel, iterations = 1)
